...
export function Mainpage({ user, setUser }) {
    ...
    const numberOfLessons = 7; // Define numberOfLessons here
    ...
    if (isTeacher) {
        const dayIndex = shortDaysOfWeek.indexOf(selectedDay) + 1;
    ...
    const handleClick = (lesson) => {
        if (lesson.startTime !== lesson.endTime && lesson.name !== 'no_lessons') {
            navigate('/lesson', { state: { lesson, date: getDateForDay(selectedDay, weekOffset) } });
        }
    };
    ...
    return (
        <div className="form5">
            ...
            <div className="week-navigation">
                <button onClick={handlePreviousWeek}>{t('previous_week')}</button>
                <button onClick={handleNextWeek}>{t('next_week')}</button>
            </div>
            <div className="days">
                <div className="week-days">
                    {shortDaysOfWeek.filter(day => !day.endsWith('7')).map((day, index) => (
                        ...
                        >
                            ...
                            <p className='dateDay' id='inter-light-font'>{getDateForDay(day, weekOffset)}</p>
                        </div>
                    ))}
                </div>
